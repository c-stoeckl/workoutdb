...
export async function getFavoriteWorkoutsAction(): Promise<
  WorkoutWithDetails[]
> {
  ...
  return (workouts as RawWorkoutFromAction[]).map(
    (workout): WorkoutWithDetails => {
      const mappedExercises: WorkoutExercise[] =
        workout.workout_exercises?.map((ex) => {
          const definition: ExerciseDefinition | null = ex.exercise_definition
            ? {
              ...
              primary_muscle_group:
                ex.exercise_definition.primary_muscle_group ?? undefined,
            }
            : null;
          ...
          return {
            ...
            weight_unit: (ex.weight_unit ?? undefined) as
              | WeightUnit
              | undefined,
    ...
    if (isCurrentlyFavorited) {
      ...
      if (insertError) {
        ...
        if (
          ...
          insertError.message?.includes("unique_user_favorite")
        ) {
          return true;
        }
